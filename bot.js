...
// import Calander reader.
const { CalanderReader } = require('./calanderReader');
...
// Const for Calander access
const MAY_I_HELP = 'may_I_help';
const LOOP_CALANDER = 'loop_calander';
...
class LoggerBot {
     ...
     * @param {CalanderUpdate} calanderUpdate A Calander object to store user's update calender
     */
    constructor(conversationState, userState) {
        ...
        // Add CalanderReader object
        this.calanderReader = new CalanderReader();
        ...
        // Create a dialog that asks if user want to start to read user's calander
        ...
        // Create a dialog that asks if user want to continue to loop through the calander items
        this.dialogs.add(new WaterfallDialog(LOOP_CALANDER, [
    ...
    async promptForStart(step) {
        return await step.prompt(START_PROMPT, 'Shall we begin by reading your calander?', ['yes', 'no']);
    }

    // This step captures the user intent to read or not his calander.
    async confirmStartPrompt(step) {
        if (step.result) {
            if (step.result.value === 'yes') {
                ...
                user.entryRead = 0;
                // Set flag to signal star reading calander
                user.startReading = 1;
                // Save userProfile
                await this.userProfile.set(step.context, user);
                // Start the calander reading
                // If there is item in the calander
                if (this.calanderReader.getTotalEntry() > -1) {
                    let calanderItem = this.calanderReader.readCalander(0);
                    await step.context.sendActivity(`on ${ calanderItem.date }
                    at ${ calanderItem.time }
                    you have ${ calanderItem.event }`);
                } else {
                    await step.context.sendActivity('You have no schedule in your calander, call me when you need me.');
    ...
    async confirmNextPrompt(step) {
        if (step.result) {
            if (step.result.value === 'yes') {
                ...
                user.loopFlag = 1;
                await this.userProfile.set(step.context, user);
                await step.beginDialog(LOOP_CALANDER);
            } else if (step.result.value === 'no') {
                await step.context.sendActivity('Understand, just call me when you want');
                return await step.endDialog();
            }
    ...
    async readCalenderItem(step) {
        ...
        for (let i = 1; i < 4; i++) {
            entryRead++;
            if (entryRead > this.calanderReader.getTotalEntry()) {
                // Stop looping as there is no more item to read
                user.startReading = 0;
                await this.userProfile.set(step.context, user);
                await step.context.sendActivity('No more schedule in your calander, call me when you need me.');
                return await step.endDialog();
            }
            let calanderItem = this.calanderReader.readCalander(entryRead);
            await step.context.sendActivity(`on ${ calanderItem.date }
            at ${ calanderItem.time }
            you have ${ calanderItem.event }`);
        }
    ...
    // This step captures the user intent to read next item on his calander.
        ...
        if (turnContext.activity.type === ActivityTypes.Message) {
            ...
            if (!turnContext.responded) {
                ...
                /* if (user.startReading === 2) {
                    // If there are more than one item in the calander, loop it
                    await dc.beginDialog(LOOP_CALANDER);
                } else {
                    await dc.beginDialog(MAY_I_HELP);
                } */
            }
